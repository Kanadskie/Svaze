...
import { Building2, Users2, Crown, CheckCircle, Handshake, ChevronDown, Users, Target, Bookmark, Award, NotebookPen, TrendingUp, HandCoins } from 'lucide-react'
...
type AccordionItem = 'description' | 'benefits' | 'tariffs'

export default function Business() {
  const [openAccordions, setOpenAccordions] = useState<Record<number, AccordionItem | null>>({})

  const toggleAccordion = (sectionIndex: number, item: AccordionItem) => {
    setOpenAccordions(prev => ({
      ...prev,
      [sectionIndex]: prev[sectionIndex] === item ? null : item
    }))
  }
      ...
      <div className="container-custom px-4 sm:px-6">
        ...
        >
          ...
          <div className="flex flex-col lg:flex-row lg:items-start lg:gap-6 xl:gap-8">
            {/* Левая колонка - Фото - ВЕРНУЛ STICKY */}
            <motion.div
            ...
            {/* Правая часть - Контент с аккордеоном */}
            <div className="lg:w-2/3 space-y-6 md:space-y-8">
              {businessSections.map((section, sectionIndex) => {
                const isDescriptionOpen = openAccordions[sectionIndex] === 'description'
                const isBenefitsOpen = openAccordions[sectionIndex] === 'benefits'
                const isTariffsOpen = openAccordions[sectionIndex] === 'tariffs'
                  ...
                  >
                    ...
                    <div className="space-y-4">
                      ...
                      <div className="border border-white/10 rounded-lg overflow-hidden">
                        <button
                          onClick={() => toggleAccordion(sectionIndex, 'description')}
                        ...
                        >
                          <div className="flex items-center gap-3 sm:gap-4">
                            <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-lg bg-white/10 flex items-center justify-center">
                              <NotebookPen className="w-4 h-4 sm:w-5 sm:h-5" style={{ color: 'var(--color-accent)' }} />
                            </div>
                            <span 
                        ...
                        <AnimatePresence>
                          {isDescriptionOpen && (
                            ...
                            >
                              {/* ДОБАВИЛ MARGIN TOP ВНУТРИ АККОРДЕОНА */}
                      ...
                      <div className="border border-white/10 rounded-lg overflow-hidden">
                        <button
                          onClick={() => toggleAccordion(sectionIndex, 'benefits')}
                          className="w-full p-4 sm:p-5 flex items-center justify-between hover:bg-white/5 transition-colors duration-200"
                        >
                          <div className="flex items-center gap-3 sm:gap-4">
                            <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-lg bg-white/10 flex items-center justify-center">
                              <TrendingUp className="w-4 h-4 sm:w-5 sm:h-5" style={{ color: 'var(--color-accent)' }} />
                            </div>
                            <span 
                              className="font-medium text-sm sm:text-base"
                              style={{ color: 'var(--color-surface)' }}
                      ...
                      <div className="border border-white/10 rounded-lg overflow-hidden">
                        <button
                          onClick={() => toggleAccordion(sectionIndex, 'tariffs')}
                          className="w-full p-4 sm:p-5 flex items-center justify-between hover:bg-white/5 transition-colors duration-200"
                        >
                          <div className="flex items-center gap-3 sm:gap-4">
                            <div className="w-8 h-8 sm:w-10 sm:h-10 rounded-lg bg-white/10 flex items-center justify-center">
                                ...
                                <div className="bg-gradient-to-r from-accent/10 to-green-fern/10 rounded-lg mb-8">
                                  <div className="flex flex-col sm:flex-row sm:items-center justify-between gap-3">
                                    <div className="flex items-center gap-2">
                                      <HandCoins className="w-5 h-5" style={{ color: 'var(--color-accent)' }} />
                                      <span 
                                        className="font-medium text-sm sm:text-base"
                                        style={{ color: 'var(--color-surface)' }}
                                      >
                                  ...
                                  <div className="grid grid-cols-1 sm:grid-cols-2 gap-3">
                                    {section.tariffs.includes.map((item, i) => (
                                      <div key={i} className="flex items-start gap-3 bg-white/5 rounded-lg p-3 sm:p-4">
                                        <Bookmark className="w-4 h-4 sm:w-5 sm:h-5 flex-shrink-0 mt-0.5" 
                                          style={{ color: 'var(--color-warm-accent)' }} />
                                        <span 
